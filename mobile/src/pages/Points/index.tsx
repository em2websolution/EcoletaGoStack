...
const Points = () => {
  ...
  // Armazenas itens inicial
  const [items, setItems] = useState<Item[]>([]);
  const [selectedItems, setSelectedItems] = useState<number[]>([]);

  // Armazena posicição inicial do usuário no mapa
  const [initialPosition, setInitialPosition] = useState<[number, number]>([0, 0]);

  // ARmazena pontos inicial
  ...
  useEffect(()=>{
    async function loadPostion() {
      ...
      const locacation = await Location.getCurrentPositionAsync();
      const { latitude, longitude } = locacation.coords;
  ...
  // Carrega os posntos de coleta cadastrados
  useEffect(()=>{
    api.get('points',{
      params: {
